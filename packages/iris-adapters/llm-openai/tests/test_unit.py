"""Unit tests for iris-adapter-llm-openai."""
...
_API_KEY = "sk-test-key"  # pragma: allowlist secret
_MODEL_CHAT = "gpt-4o-mini"
_MODEL_EXTRACT = "gpt-4o"
...
def _make_provider(response: httpx.Response | None = None) -> OpenAIProvider:
...
def test_base_url_is_openai() -> None:
    provider = _make_provider()
...
def test_length_finish_reason_raises_context_window() -> None:
    ...
    with pytest.raises(LLMContextWindowExceeded, match="openai") as exc_info:
        _run(provider._do_complete(_CTX, _REQ))
...
def test_timeout_raises_unavailable() -> None:
    ...
    provider = OpenAIProvider(api_key=_API_KEY, _http_client=mock_client)
...
def test_from_env_raises_on_missing_var(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("IRIS_LLM_OPENAI_API_KEY", raising=False)
    with pytest.raises(RuntimeError, match="IRIS_LLM_OPENAI_API_KEY"):
