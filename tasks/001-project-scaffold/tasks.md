...
- [ ] **T008** `[P] [US2] [size: M] [owner: AuroPro]` Add `.github/workflows/ci.yml` (or GitLab equivalent) running `make lint`, `make typecheck`, `make test`, `make test-cov` on `pull_request` and `push` to `main`.
